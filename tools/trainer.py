...
def main():
    for model_type in ["yolov8s_pMCA_C2PSSA_GD"]:
        print(f"Training {model_type}:")

        model = YOLO(model_type+".yaml")
        # model = YOLO("ChickenCounting/yolov8s_MCA_AConv_C2PSA_GD_100e_coco2/weights/best.pt")
        model.train(data='./data/data.yaml', epochs=350, batch=0.80, cache=True, project=f'ChickenCounting', name=f"{model_type}", plots=True, pretrained=False)
