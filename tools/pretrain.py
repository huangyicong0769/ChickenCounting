...
def main():
    for model_type in ["yolov8s_GD"]:
        print("Training "+model_type+":")
        model = YOLO(model_type+".yaml")
        model.train(data="coco.yaml", epochs=100, batch=0.85, cache=False, project='ChickenCounting', name=model_type+"_100e_coco", plots=False)
